...
exports.renderDataInvoice = renderDataInvoice;
exports.renderDataTotal = renderDataTotal;
const axios_1 = __importDefault(require("axios"));
const utils_1 = require("./utils");
async function generateHeader(doc, data, config) {
...
function renderShapes(doc, libre) {
    doc.lineWidth(0.5);
    if (!libre)
...
function renderItems(doc, data, currency, rate) {
    ...
    data.map((item) => {
        ...
        doc
            ...
            .text((0, utils_1.numFormat)(price), 405, y, { width: 70, align: "right" })
            .text((0, utils_1.numFormat)(itemTotal), 485, y, {
            width: 70,
            align: "right",
        });
        y += 10;
...
function renderDataInvoice(doc, customer, date, number, currency, libre) {
    ...
    doc
        .fontSize(9)
        .text(`Nombre o Razón Social: ${customer.name} ${customer.lastname}`, 40, 157)
...
function renderDataTotal(doc, data, number, currency, libre) {
    ...
    doc
        ...
        .text((0, utils_1.numFormat)(data.subtotal, currency), 485, 688, {
        width: 70,
        align: "right",
    })
        .text(`I.V.A. ${data.iva}% SOBRE ${(0, utils_1.numFormat)(data.baseImponible, currency)}`, 325, 705, { width: 150, align: "right" })
        .text((0, utils_1.numFormat)(totalIva, currency), 485, 705, {
        width: 70,
        align: "right",
    })
        .font("Times-Bold")
        .text(`TOTAL A PAGAR`, 395, 720)
        .text((0, utils_1.numFormat)(total, currency), 485, 720, { width: 70, align: "right" });
